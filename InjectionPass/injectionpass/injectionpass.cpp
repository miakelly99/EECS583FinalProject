  ...
  PreservedAnalyses run(Module &M, ModuleAnalysisManager &MAM) {
	  ...
	  for (Function &F: M.getFunctionList())
    {
      ...
      for (LoadInst* load_inst: load_insts)
      {
        ...
        Instruction* current_target = load_inst;

        for (auto injection_pair: inj_map)
        {
          LoadInst* add_at_load_inst = load_inst;
          ...
          if (instruction_num != 0)
          {
            ...
            if (remote_location != load_inst)
            {
              ...
              LoadInst* remote_load_value = new LoadInst(load_inst->getType(), load_inst->getPointerOperand(), "remote_injection", remote_location->getNextNode());

              add_at_load_inst = remote_load_value;
            }
          }
          ...
          Instruction* load_iteration_num = new LoadInst(iteration_number_type, iteration_number, "", add_at_load_inst->getNextNode());
          ...
          Instruction* corrupt_value = BinaryOperator::Create(Instruction::Xor, add_at_load_inst, load_inj_value, "", load_inj_value->getNextNode());
          Instruction* inject_corrupt_value = new StoreInst(corrupt_value, add_at_load_inst->getPointerOperand(), corrupt_value->getNextNode());
          ...
          phi_node_1->addIncoming(add_at_load_inst, icmp_iter_nums->getParent());
          ...
          phi_node_2->addIncoming(add_at_load_inst, add_at_load_inst->getParent());
          phi_node_2->addIncoming(phi_node_1, phi_node_1->getParent());

          current_target->replaceUsesWithIf(phi_node_2,
            [init_block = add_at_load_inst->getParent(), inject_block = corrupt_value->getParent(), phi_node_1 = phi_node_1, phi_node_2 = phi_node_2](Use& u) {
              bool val = false;
          ...
          current_target = (Instruction*) phi_node_2;
        }
      }
    }
    template_file.close();
